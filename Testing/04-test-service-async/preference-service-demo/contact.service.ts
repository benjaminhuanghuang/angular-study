...
export class ContactService {
  ...
  public getContact(id: number): Observable<Contact> {
    return this.getContacts().map((contacts) => contacts.find((contact) => contact.id === id));
  }
  ...
  public post(contact: Contact): Observable<Contact> {
    return this.http.post(this.contactsUrl, JSON.stringify(contact), { headers: this.headers }).catch(this.handleError);
  }
